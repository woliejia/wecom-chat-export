...
3. **Fetch the tool scripts** (ylytdeng/wechat-decrypt is DMCA-taken-down;
   use the live mirror `kenchikuliu/wechat-local-backup-search`). Download via
   `https://gh-proxy.com/https://raw.githubusercontent.com/kenchikuliu/wechat-local-backup-search/main/(file)`
   for: `wxwork_crypto.py`, `find_wxwork_keys.py`, `key_scan_common.py`,
   `key_utils.py`, `decrypt_wxwork_db.py`, `export_wxwork_messages.py`, `config.py`
   (substitute each file name for the `(file)` placeholder in the URL).
   (Plain raw-file fetches through gh-proxy work even when archive/zip does not.)
4. **config.json**: MUST set `wxwork_db_dir` explicitly to the `Data` folder
   (auto-detect only checks `%USERPROFILE%\Documents\WXWork`, which fails after the
   Huawei relocation). Also set `wxwork_decrypted_dir` and `wxwork_export_dir`.
   See `references/algorithm.md` for a ready config.
